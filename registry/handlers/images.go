...
func (imh *imageManifestHandler) GetImageManifest(w http.ResponseWriter, r *http.Request) {
	...
	// matching the digest.
	if schema2Manifest, isSchema2 := manifest.(*schema2.DeserializedManifest); imh.Tag != "" && isSchema2 && !supportsSchema2 {
	...
	} else if manifestList, isManifestList := manifest.(*manifestlist.DeserializedManifestList); imh.Tag != "" && isManifestList && !supportsManifestList {
		// Rewrite manifest in schema1 format
		...
		for _, manifestDescriptor := range manifestList.Manifests {
			if manifestDescriptor.Architecture == defaultArch && manifestDescriptor.OS == defaultOS {
