...
func testManifestAPISchema2(t *testing.T, env *testEnv, imageName string) manifestArgs {
	...
	if !bytes.Equal(fetchedManifest.Canonical, canonical) {
		t.Fatalf("manifests do not match")
	}
	...
	if !bytes.Equal(fetchedManifestByDigest.Canonical, canonical) {
		t.Fatalf("manifests do not match")
	}
...
func testManifestAPIManifestList(t *testing.T, env *testEnv, args manifestArgs) {
	...
	if !bytes.Equal(fetchedManifestList.Canonical, canonical) {
	...
	if !bytes.Equal(fetchedManifestListByDigest.Canonical, canonical) {
		t.Fatalf("manifests do not match")
	}
